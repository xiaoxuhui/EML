...
class MainActivity : ComponentActivity() {
    ...
    private fun buildWebView(): WebView {
        ...
        view.setBackgroundColor(Color.parseColor("#EEF2F4"))
        ...
        view.addJavascriptInterface(Bridge(), "EMLAndroid")
        return view
    }
    ...
    private fun sanitizeName(name: String): String {
        ...
        val fallback = "eml-workbench-export.json"
    ...
    companion object {
        ...
        private val EXPORT_BRIDGE_JS = """
            (function () {
              if (window.__emlAndroidBridge) { return; }
              window.__emlAndroidBridge = true;
              document.addEventListener('click', function (event) {
                ...
                var name = node.getAttribute('download') || 'eml-workbench-export.json';
                fetch(href).then(function (response) { return response.text(); }).then(function (text) {
                  window.EMLAndroid.saveFile(name, text);
                }).catch(function () {
                  window.EMLAndroid.saveFile(name, '');
                });
              }, true);
            })();
        """.trimIndent()
